...
export default function PortalPaymentReceiptPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const [payment, setPayment] = useState<Payment | null>(null);

  useEffect(() => {
    void api.get<Payment>(`/payments/me/${id}/receipt`).then((res) => setPayment(res.data));
  }, [id]);
